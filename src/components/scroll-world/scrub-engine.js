...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      const c = copies[i];
      c.style.opacity = cop;
      // Preserve the CSS centering: the stylesheet centers the copy with
      // translateY(-50%) (desktop), but an inline transform replaces it —
      // without the -50% the block hangs from mid-screen and reads too low.
      // Mobile anchors via bottom (no centering), so no -50% there.
      const centered = !isMobile();
      c.style.transform = reduce
        ? (centered ? 'translateY(-50%)' : 'none')
        : `translateY(calc(${centered ? '-50%' : '0vh'} + ${(0.5 - pr) * 4}vh))`;
